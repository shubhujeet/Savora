...
function addFilters() {
  ...
  for (let nm of storedFilters) {
    // console.log(i)
    createFilterTemplate(nm);
  }
}

addFilters();

function createFilterTemplate(name) {
...
searchBarInp.addEventListener("keypress", (e) => {
  searchRecipe(searchBarInp.value.toLowerCase());
  searchRecipe2();
});
...
const searchRecipe2 = (name = null) => {
  let searchVal;
  if (!name) {
    searchVal = searchBarInp.value.toLowerCase();
  } else {
    searchVal = name;
  }
  ...
  function selectDay(week) {
    ...
    for (let nm of weekName) {
      ...
      wnodeNm.addEventListener("click", (e) => {
        ...
        if (recpieNameFound) {
          alert(`Recipe is already added to your ${week} ${nm} plan`);
        } else {
